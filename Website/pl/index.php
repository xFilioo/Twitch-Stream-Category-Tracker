...
<header>
    ...
    <form method="get" id="filters">
      ...
      <label>Od: <input type="date" name="start" min="2025-07-08" value="<?= $start ?>"></label>
      <label>Do: <input type="date" name="end" value="<?= $end ?>"></label>
      <button type="submit">Pokaż</button>
    </form>
    <?php if ($summary): ?>
    <div class="summary">
      <div><span>Pozycja:</span> <?= $summary['rank'] ?></div>
      <div><span>Minut streamowanych:</span> <?= $summary['minutes_streamed'] ?></div>
      <div><span>Śr. widzowie:</span> <?= $summary['avg_viewers'] ?></div>
      <div><span>Maks. widzowie:</span> <?= $summary['max_viewers'] ?></div>
      <div><span>Godzin obejrzanych:</span> <?= $summary['hours_watched'] ?></div>
      <div><span>Nowi follow:</span> <?= $summary['followers'] ?></div>
      <div><span>Łącznie follow:</span> <?= $summary['followers_total'] ?></div>
    </div>
    <?php endif; ?>
  </header>
  <main>
    <h2>Kategorie streamów <span class="datetitle">(<?= $start ?> – <?= $end ?>)</span></h2>
    <div class="switcher">
      <button type="button" id="showTable" class="active">Tabela</button>
      <button type="button" id="showChart">Wykres</button>
    </div>
    <div id="tableWrap">
      <table id="categories" class="active">
        <thead>
          <tr>
            <th onclick="sortTable(0)">Kategoria</th>
            <th onclick="sortTable(1)">Minut</th>
            <th onclick="sortTable(2)">% czasu</th>
          </tr>
        </thead>
        <tbody>
      ...
      <?php if (empty($stats)): ?>
        <div class="nodata">Brak danych w wybranym zakresie.</div>
      <?php endif; ?>
    </div>
    <div id="chartWrap">
